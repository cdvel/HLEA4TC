...
import co.velandia.hlea4tc.agents.junction.JunctionUpdateBean;
...
public class PlatformMediator {
    ...
    static ArrayList<AgentController> jControllers = new ArrayList<
        AgentController
    >();
    static ArrayList<AgentController> sControllers = new ArrayList<
        AgentController
    >();
    ...
    public static boolean startJadePlatform(String junctions) {
        ...
        try {
            ...
            pContainer.setParameter(
                Profile.CONTAINER_NAME,
                "Traffic-Agents-Container"
            );
            ...
            Logger.getLogger(PlatformMediator.class.getName()).log(
                Level.SEVERE,
                null,
                ex
            );
    ...
    public static boolean initJunctionAgent(String id) {
        if (container != null) {
            try {
                AgentController jun = container.createNewAgent(
                    junctionPrefix + id,
                    "co.velandia.hlea4tc.agents.JunctionAgent",
                    new Object[0]
                );
                jun.start();
    ...
    public static boolean initSectorAgent(String id) {
        if (container != null) {
            try {
                AgentController sec = container.createNewAgent(
                    sectorPrefix + id,
                    "co.velandia.hlea4tc.agents.SectorAgent",
                    new Object[0]
                );
                sec.start();
    ...
    public static String updJunctionAgent(String values) {
        if (container != null) {
            try {
                ...
                AgentController tAgent = container.getAgent(
                    junctionPrefix + junctionID,
                    true
                ); // TODO: untested, get by GUID instead of local name
                tAgent.putO2AObject(
                    new JunctionUpdateBean(junctionID),
                    AgentController.ASYNC
                ); // TODO: check for SYNC version!
                return "101";
            } catch (StaleProxyException ex) {
                Logger.getLogger(PlatformMediator.class.getName()).log(
                    Level.SEVERE,
    ...
    public static String OptAPOinitialise(String junctions, String incoming) {
        if (container != null) {
            try {
                ...
                JunctionUpdateBean junctBean = new JunctionUpdateBean(
                    junctionID
                );
    ...
    public static String OptAPOcheckAgentView(String junctionID)
        ...
        if (container != null) {
            ...
            junctionBean.coordination = junction.myState.junctionID;
        }
        return null;
    }
}
