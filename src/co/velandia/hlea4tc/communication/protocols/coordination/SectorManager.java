package co.velandia.hlea4tc.communication.protocols.coordination;
...
public class SectorManager extends SubscriptionInitiator {
    ...
    protected void handleInform(ACLMessage inform) {
        try {
            DFAgentDescription[] dfds = DFService.decodeNotification(
                inform.getContent()
            );

            for (int i = 0; i < dfds.length; i++) {
                if (
                    !dfds[i].getName().equals(myAgent.getAID())
                ) { // know thyself TODO: unneeded here, but when refactoring; only sectors register
