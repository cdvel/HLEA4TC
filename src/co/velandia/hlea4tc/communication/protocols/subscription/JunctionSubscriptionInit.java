...
public class JunctionSubscriptionInit extends SubscriptionInitiator {
    ...
    protected void handleAllResponses(Vector responses) {
        ...
        System.out.println(
            "[J] " + myAgent.getLocalName() + "\t <----------- NOTIFIED"
        );
        for (int i = 0; i < responses.size(); i++) {
            ...
            if (thisMsg.getPerformative() == ACLMessage.ACCEPT_PROPOSAL) {
                System.out.println(
                    ...
                    myAgent.getLocalName() +
                    "\t ! SUBSCRIBED to " +
                    thisMsg.getSender().getLocalName()
                );
            }
        ...
        if (mySector == null) {
            ...
            if (!mySector.getName().equals(agree.getSender().getName())) {
                ...
                System.out.println(
                    "Junction " +
                    myAgent.getLocalName() +
                    ": Reallocated from Sector " +
                    oldSector.getLocalName() +
                    " to " +
                    mySector.getLocalName()
                );
    ...
    protected void handleAllResultNotifications(ArrayList notifications) {
        if (notifications.size() < knownSectors.size()) {
            ...
            System.out.println(
                ...
                (knownSectors.size() - notifications.size()) +
                " responses"
            );
        }
    }
