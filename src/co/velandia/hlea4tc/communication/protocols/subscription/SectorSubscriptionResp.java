package co.velandia.hlea4tc.communication.protocols.subscription;
...
public class SectorSubscriptionResp extends SubscriptionResponder {
     ...
     */
    @Override
    protected ACLMessage handleSubscription(ACLMessage subscriptionMsg)
        throws RefuseException {
        System.out.println(
            myAgent.getLocalName() +
            ": Received subscription request from " +
            subscriptionMsg.getSender().getLocalName() +
            " [" +
            subscriptionMsg.getContent() +
            "]"
        );
        ...
        if (junctionMatches(subscriptionMsg)) {
            ...
            if (junctionAwarded(subscriptionMsg)) {
                ...
                Vector previousSub = getSubscriptions(
                    subscriptionMsg.getSender()
                ); //
                if (
                    previousSub.size() == 0
                ) { // one subscriptionMsg per junction
                    createSubscription(subscriptionMsg);
                }
                System.out.println(
                    myAgent.getLocalName() +
                    ": AGREED subscription request from " +
                    subscriptionMsg.getSender().getLocalName() +
                    " [" +
                    subscriptionMsg.getContent() +
                    "]"
            ...
            System.out.println(
                myAgent.getLocalName() +
                ": REFUSED subscription request from " +
                subscriptionMsg.getSender().getLocalName() +
                " [" +
                subscriptionMsg.getContent() +
                "]"
    ...
    private void storeNotification(int performative) {
        if (performative == ACLMessage.SUBSCRIBE) {
            System.out.println(
                "Agent " + myAgent.getLocalName() + ": subscription successful"
            );
        }
    ...
    public void notifyJunctions(ACLMessage inform) {
        ...
        for (int i = 0; i < subs.size(); i++) {
            ((SubscriptionResponder.Subscription) subs.elementAt(i)).notify(
                    inform
                );
        }
    }
